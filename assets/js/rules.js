...
OBRAS.rules = {
  ...
  obraMetrics: function(obra, db){
    var recebimentos = db.recebimentos.filter(function(r){ return r.os === obra.numeroOS; });
    var totalRecebido = recebimentos.reduce(function(s,r){ return s + Number(r.valor || 0); }, 0);
    var repasse = db.repasses.find(function(r){ return r.os === obra.numeroOS; });
    var valorFechadoParceiro = Number(repasse && repasse.valorFechadoParceiro || 0);
    var pagamentos = db.pagamentosParceiros.filter(function(p){ return p.os === obra.numeroOS; });
    var totalPagoParceiro = pagamentos.reduce(function(s,p){ return s + Number(p.valor || 0); }, 0);
    var despesasObra = db.despesas.filter(function(d){ return d.os === obra.numeroOS; });
    var totalDespesasObra = despesasObra.reduce(function(s,d){ return s + Number(d.valor || 0); }, 0);
    var despesasPendentes = despesasObra.filter(function(d){ return !d.dataPagamentoReal; }).reduce(function(s,d){ return s + Number(d.valor || 0); }, 0);
    var saldoReceber = Math.max(0, Number(obra.valorObra || 0) - totalRecebido);
    var saldoParceiro = Math.max(0, valorFechadoParceiro - totalPagoParceiro);
    var statusFinanceiro = saldoReceber === 0 ? 'Quitado' : (totalRecebido > 0 ? 'Recebido parcial' : 'A receber');
    var statusCicloOS = obra.statusCicloOS || 'Ativa';
    return {
      id: obra.id,
  ...
  painelMetrics: function(db){
    ...
    var valorFechadoParceiros = obras.reduce(function(s,o){ return s + Number(o.valorFechadoParceiro || 0); }, 0);
    var valorPagoParceiros = db.pagamentosParceiros.filter(function(p){ return p.dataPagamentoReal; }).reduce(function(s,p){ return s + Number(p.valor || 0); }, 0);
    var saldoParceirosAPagar = Math.max(0, valorFechadoParceiros - db.pagamentosParceiros.reduce(function(s,p){ return s + Number(p.valor || 0); }, 0));
    var despesasObraPagas = db.despesas.filter(function(d){ return d.dataPagamentoReal; }).reduce(function(s,d){ return s + Number(d.valor || 0); }, 0);
    var despesasObraPendentes = db.despesas.filter(function(d){ return !d.dataPagamentoReal; }).reduce(function(s,d){ return s + Number(d.valor || 0); }, 0);
    var despesasGeraisPagas = db.despesasGerais.filter(function(d){ return d.dataPagamentoReal; }).reduce(function(s,d){ return s + Number(d.valor || 0); }, 0);
    var despesasGeraisPendentes = db.despesasGerais.filter(function(d){ return !d.dataPagamentoReal; }).reduce(function(s,d){ return s + Number(d.valor || 0); }, 0);
    ...
    var despesasAVencer = db.despesas.filter(function(d){ return !d.dataPagamentoReal && OBRAS.helpers.statusVencimento(d.dataVencimento,'') !== 'Atrasado'; }).reduce(function(s,d){ return s + Number(d.valor || 0); }, 0) +
      db.despesasGerais.filter(function(d){ return !d.dataPagamentoReal && OBRAS.helpers.statusVencimento(d.dataVencimento,'') !== 'Atrasado'; }).reduce(function(s,d){ return s + Number(d.valor || 0); }, 0);
    var despesasAtrasadas = db.despesas.filter(function(d){ return !d.dataPagamentoReal && OBRAS.helpers.statusVencimento(d.dataVencimento,'') === 'Atrasado'; }).reduce(function(s,d){ return s + Number(d.valor || 0); }, 0) +
      db.despesasGerais.filter(function(d){ return !d.dataPagamentoReal && OBRAS.helpers.statusVencimento(d.dataVencimento,'') === 'Atrasado'; }).reduce(function(s,d){ return s + Number(d.valor || 0); }, 0);
    return {
      obras: obras,
  ...
  avisos: function(db, m){
    ...
    var vencendo = db.despesas.concat(db.despesasGerais).filter(function(item){
      return !item.dataPagamentoReal && item.dataVencimento && OBRAS.helpers.compareISODate(item.dataVencimento, hoje) >= 0;
    });
